...
def main() -> NoReturn:
    """Main CLI loop with dependency injection."""
    config = Config()
    ...
    while True:
        ...
        try:
            if choice == "1":
                ...
                project = project_service.create_project(name, desc or "")
                ...
                task = task_service.create_task(pid, title, desc or "", status_input, deadline)  # type: ignore[arg-type]
                print("Task created:")
                print_task(task)
            elif choice == "6":
                pid_int = int(get_input("Project ID: ") or "")
                ...
                task = task_service.change_task_status(pid, tid, new_status)  # type: ignore[arg-type]
                ...
                if all(x is None for x in [new_title, new_desc, new_status, new_deadline]):
                    print("No changes made.")
                    continue
                task = task_service.edit_task(pid, tid, new_title, new_desc, new_status, new_deadline)  # type: ignore[arg-type]
                print("Task updated:")
                print_task(task)
            elif choice == "8":
                pid_int = int(get_input("Project ID: ") or "")
